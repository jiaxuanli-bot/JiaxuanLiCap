...
//    }
    ...
    public User modifyUser(@Pattern( regexp = "^[0-9]*$", message = "the UserID format is wrong") @PathVariable String id, User user) {
        User u = userRepo.findByIdEquals(Long.valueOf(id));
        return userServiceNew.modifyUser(user, u);
    }
...
//    }
    ...
    public List<CommitteeSummary> getUserCommittees(@Pattern( regexp = "^[0-9]*$", message = "the UserID format is wrong") @PathVariable String id, @Pattern( regexp = "\\b\\d{4}\\b", message = "the year format is wrong") @RequestParam(name="year", required=false) String year) {
        //Get all the committees that user have been assigned
        User u =new User();
        u.setId(Long.valueOf(id));
        return  userServiceNew.getUserCommittees(u, year);
    }
